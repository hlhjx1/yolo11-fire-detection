...
## <div align="center">项目说明</div>

本仓库是在 Ultralytics YOLO11 基础上整理的森林火灾检测项目，面向论文 **A New Collaborative Detection Method for Forest Fires Under Degraded Image Conditions** 的复现、实验与展示。

论文提出了 **CoDeF-Net** 协同检测框架，将 **Retinex-BCE 图像增强** 与 **YOLOv11** 结合，用于提升雾霾、低照度和其他退化图像条件下的森林火灾检测能力。项目中的核心目标是增强烟雾与火焰的可见性，并提高小火焰、远距离火点和复杂背景下的检测稳定性。

### 主要内容

- **Retinex-BCE 增强**：基于 Retinex 的亮度分解与亮通道增强，改善退化图像质量。
- **协同检测**：增强图像再输入 YOLOv11，实现检测与增强协同优化。
- **结果分析**：仓库中的结果绘图脚本可用于训练曲线、精度曲线和增强对比分析。
...
如果这是你自己的项目分支，通常需要先在 GitHub 上创建一个空仓库，然后把本地代码推上去：

```bash
git init
git add .
git commit -m "init: add fire detection project"
git branch -M main
git remote add origin <你的GitHub仓库地址>
git push -u origin main
```

如果 GitHub 仓库已经创建好了，就直接执行后面的 `remote add` 和 `push` 即可。
